Fabricator(:market) do
  ...
  sport_id { (Sport.find_by(ext_sport_id: 1) || Fabricate(:football)).id }
end
...
Fabricator(:market_basketball_money_line, from: :market) do
  ext_market_id { 219 }
  name { "Money Line" }
  sport_id { (Sport.find_by(ext_sport_id: 2) || Fabricate(:basketball)).id }
end
...
Fabricator(:market_tennis_match_winner, from: :market) do
  ext_market_id { 186 }
  name { "Match Winner" }
  sport_id { (Sport.find_by(ext_sport_id: 5) || Fabricate(:tennis)).id }
end
